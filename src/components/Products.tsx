...
export function Products() {
  ...
  const getFeaturesList = (detailDescription: string) => {
    ...
    return detailDescription.split(",").map((feature, index) => (
      <li key={index} className={`${darkMode.value ? "text-white" : "text-black"}`}>{feature.trim()}</li>
    ));
  };
  ...
  return (
    <div id='products' className="min-h-screen w-full dark:bg-black bg-white  dark:bg-grid-white/[0.2] bg-grid-black/[0.2] relative flex items-center justify-center">
      ...
      <div className="relative z-20 py-8 w-full max-w-[95%] mx-auto md:max-w-4xl lg:max-w-5xl xl:max-w-6xl px-4 lg:px-8 xl:px-12">
        <h1 className={`text-4xl md:text-5xl lg:text-6xl text-center mb-8 ${darkMode.value ? "text-white" : "text-black"}`}>Products</h1>
        <Box sx={{ width: "100%" }}>
          <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
            <Tabs value={value} onChange={handleChange} aria-label="basic tabs example" variant="scrollable" scrollButtons="auto" centered>
              {productsData.map((brandData: BrandData, index: number) => (
                <Tab key={brandData.brand} label={brandData.brand} {...a11yProps(index)} className={`${darkMode.value ? "text-white" : "text-black"} text-base sm:text-lg md:text-xl lg:text-2xl`} />
              ))}
            </Tabs>
          </Box>
          {productsData.map((brandData: BrandData, index: number) => (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mx-auto">
                {brandData.products.map((product: Product) => (
                  <div key={product.title} className="bg-white dark:bg-gray-800 shadow-md rounded-lg p-4 transition-transform transform hover:scale-105">
                    ...
                    <h3 className={`text-lg md:text-xl font-semibold ${darkMode.value ? "text-white" : "text-black"}`}>{product.title}</h3>
                    <p className={`mb-2 ${darkMode.value ? "text-white" : "text-black"}`}>{product.briefDescription}</p>
                    <Button className={`font-semibold mb-2 focus:outline-none ${darkMode.value ? "text-blue-300" : "text-blue-500"}`} onClick={() => handleLearnMore(product)}>Learn More</Button>
                  </div>
                ))}
              </div>
            </CustomTabPanel>
        ...
        <Drawer anchor="right" open={drawerOpen} onClose={handleDrawerClose} PaperProps={{ sx: { backgroundColor: darkMode.value ? "rgba(0, 0, 0, 0.8)" : "rgba(255, 255, 255, 0.8)", color: darkMode.value ? "white" : "black", width: "300px", padding: "16px" } }}>
          {selectedProduct && (
        ...
        <Modal open={modalOpen} onClose={() => setModalOpen(false)} aria-labelledby="modal-title" aria-describedby="modal-description">
          <Box sx={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: 300, bgcolor: darkMode.value ? "rgba(0, 0, 0, 0.8)" : "white", color: darkMode.value ? "white" : "black", boxShadow: 24, p: 4, borderRadius: 2 }}>
            ...
            <Typography id="modal-description" sx={{ mt: 2 }}>Call now for better deals: +91 888</Typography>
          </Box>
        </Modal>
      </div>
    </div>
